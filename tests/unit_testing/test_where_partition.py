...
class DfsWhereTest(unittest.TestCase):
    ...
    @property
    def odfs_csv(self):
        return Csv.odfs_csv.set_index("date")
    ...
    @property
    def odfs_re(self):
        return Csv.odfs_csv.set_index("PERMNO")
    ...
    def test_dfs_from_pandas_where_param_cond_equal(self):
        # where ==
        a = self.odfs[self.odfs["tsymbol"] == 'a'].to_pandas()
        b = self.pdf[self.pdf["tsymbol"] == 'a']
        assert_frame_equal(a, b, check_dtype=False)

    def test_dfs_from_pandas_where_param_cond_not_equal(self):
        # where !=
        a = self.odfs[self.odfs["tsymbol"] != 'a'].to_pandas()
        b = self.pdf[self.pdf["tsymbol"] != 'a']
        assert_frame_equal(a, b, check_dtype=False)

    def test_dfs_from_pandas_where_param_cond_less_than_or_equal(self):
        # where <=
        a = self.odfs[self.odfs["tfloat"] <= 3.0].to_pandas()
        b = self.pdf[self.pdf["tfloat"] <= 3.0]
        assert_frame_equal(a, b, check_dtype=False)

    def test_dfs_from_pandas_where_param_cond_greater_than_or_equal(self):
        #  where >=
        a = self.odfs[self.odfs["tfloat"] >= 3.0].to_pandas()
        b = self.pdf[self.pdf["tfloat"] >= 3.0]
        assert_frame_equal(a, b, check_dtype=False)

    def test_dfs_from_pandas_where_param_cond_less_than(self):
        #  where <
        a = self.odfs[self.odfs["tfloat"] < 3.0].to_pandas()
        b = self.pdf[self.pdf["tfloat"] < 3.0]
        assert_frame_equal(a, b, check_dtype=False)

    def test_dfs_from_pandas_where_param_cond_greater_than(self):
        #  where >
        a = self.odfs[self.odfs["tfloat"] > 3.0].to_pandas()
        b = self.pdf[self.pdf["tfloat"] > 3.0]
        assert_frame_equal(a, b, check_dtype=False)

    def test_dfs_from_pandas_where_and(self):
        #  where  &
        odfs = self.odfs
        pdf = self.pdf
    ...
    def test_dfs_from_pandas_where_and_and(self):
        #  where & &
        odfs = self.odfs
        pdf = self.pdf
    ...
    def test_dfs_from_pandas_where_and_or(self):
        #  where & |
        odfs = self.odfs
        pdf = self.pdf
    ...
    def test_dfs_from_pandas_where_or(self):
        #  where |
        odfs = self.odfs
        pdf = self.pdf
    ...
    def test_dfs_from_pandas_where_or_and(self):
        #  where | &
        odfs = self.odfs
        pdf = self.pdf
        a = odfs[(odfs["tfloat"] == 3.0) | (odfs["tfloat"] > 1.0) & (odfs["tint"] > 2)].to_pandas()
    ...
    def test_dfs_from_pandas_where_or_or(self):
        #  where | |
        odfs = self.odfs
        pdf = self.pdf
        a = odfs[(odfs["tfloat"] < 3.0) | (odfs["tfloat"] > 1.0) | (odfs["tfloat"] == 4.0)].to_pandas()
    ...
    def test_dfs_from_pandas_where_operation_add(self):
        #  where +
    ...
    def test_dfs_from_pandas_where_operation_sub(self):
        #  where -
        odfs_dr = self.odfs_dr
        pdf_dr = self.pdf_dr
        a = (odfs_dr[odfs_dr["tfloat"] < 3.0] - odfs_dr[odfs_dr["tfloat"] > 1.0]).to_pandas()
    ...
    def test_dfs_from_pandas_where_operation_mul(self):
        #  where *
        odfs_dr = self.odfs_dr
        pdf_dr = self.pdf_dr
        a = (odfs_dr[odfs_dr["tfloat"] < 3.0] * odfs_dr[odfs_dr["tfloat"] > 1.0]).to_pandas()
    ...
    def test_dfs_from_pandas_where_operation_div(self):
        #  where /
        odfs_dr = self.odfs_dr
        pdf_dr = self.pdf_dr
        a = (odfs_dr[odfs_dr["tfloat"] < 3.0] / odfs_dr[odfs_dr["tfloat"] > 1.0]).to_pandas()
    ...
    def test_dfs_from_pandas_where_operation_groupby(self):
        #  where groupby sum
    ...
    def test_dfs_from_pandas_where_operation_resample(self):
    ...
    def test_dfs_from_pandas_where_operation_rolling(self):
    ...
    def test_dfs_from_import_where_param_cond_less_than_or_equal(self):
        # where <=
    ...
    def test_dfs_from_import_where_param_cond_greater_than_or_equal(self):
        #  where >=
        a = self.odfs_csv[self.odfs_csv["PRC"] >= 10.25].to_pandas().sort_values(['PERMNO', "TICKER"]).reset_index(
            drop=True).fillna(
            value="")
    ...
    def test_dfs_from_import_where_param_cond_less_than(self):
        #  where <
        a = self.odfs_csv[self.odfs_csv["PRC"] < 10.25].to_pandas().sort_values(['PERMNO', "TICKER"]).reset_index(
            drop=True).fillna(
            value="")
    ...
    def test_dfs_from_import_where_param_cond_greater_than(self):
        #  where >
        a = self.odfs_csv[self.odfs_csv["PRC"] > 10.25].to_pandas().sort_values(['PERMNO', "TICKER"]).reset_index(
            drop=True).fillna(
            value="")
    ...
    def test_dfs_from_import_where_and(self):
        #  where  &
    ...
    def test_dfs_from_import_where_or(self):
        #  where |
    ...
    def test_dfs_from_import_where_and_and(self):
        #  where & &
    ...
    def test_dfs_from_import_where_and_or(self):
        # where &
        a = self.odfs_csv[(self.odfs_csv["OPENPRC"] > 40) & (self.odfs_csv["SHROUT"] > 15000) | (
    ...
    def test_dfs_from_import_where_or_or(self):
        #  where | |
        a = self.odfs_csv[(self.odfs_csv["OPENPRC"] > 40) | (self.odfs_csv["SHROUT"] > 15000) | (
                    self.odfs_csv["TRDSTAT"] == "A")].to_pandas()
        b = self.pdf_csv[
    ...
    def test_dfs_from_import_where_or_and(self):
        #  where | &
        a = self.odfs_csv[(self.odfs_csv["OPENPRC"] > 40) | (self.odfs_csv["SHROUT"] > 15000) & (
                    self.odfs_csv["TRDSTAT"] == "A")].to_pandas()
        b = self.pdf_csv[
    ...
    def test_dfs_from_import_where_operation_add(self):
        #  where +
    ...
    def test_dfs_from_import_where_operation_sub(self):
        #  where -
        a = (self.odfs_d[self.odfs_d["OPENPRC"] > 40] - self.odfs_d[self.odfs_d["SHROUT"] > 15000]).to_pandas()
        b = self.pdf_d[self.pdf_d["OPENPRC"] > 40] - self.pdf_d[self.pdf_d["SHROUT"] > 15000]
        # TODO: ASSERT RANDOM ORDER
    ...
    def test_dfs_from_import_where_operation_mul(self):
        #  where *
        a = (self.odfs_d[self.odfs_d["OPENPRC"] > 40] * self.odfs_d[self.odfs_d["SHROUT"] > 15000]).to_pandas()
        b = self.pdf_d[self.pdf_d["OPENPRC"] > 40] * self.pdf_d[self.pdf_d["SHROUT"] > 15000]
        # TODO: ASSERT RANDOM ORDER
    ...
    def test_dfs_from_import_where_operation_div(self):
        #  where /
        a = (self.odfs_d[self.odfs_d["OPENPRC"] > 40] / self.odfs_d[self.odfs_d["SHROUT"] > 15000]).to_pandas()
        b = self.pdf_d[self.pdf_d["OPENPRC"] > 40] / self.pdf_d[self.pdf_d["SHROUT"] > 15000]
        # TODO: ASSERT RANDOM ORDER
    ...
    def test_dfs_from_import_where_operation_groupby(self):
        #  where groupby sum
    ...
    def test_dfs_from_import_where_operation_resample(self):
    ...
    def test_dfs_from_import_where_operation_rolling(self):

...
- [将数据导出到磁盘](#1-将数据导出到磁盘)
- [将数据保存到DolphinDB数据表](#2-将数据保存到dolphindb数据表)
  - [保存数据到Orca内存表](#21-保存数据到orca内存表)
  - [保存数据到Orca磁盘表](#22-保存数据到orca磁盘表)
  - [保存数据到Orca分布式表](#23-保存数据到orca分布式表)
- [小结](#3-小结)
...
- 磁盘表
   ...
   通过`read_table`函数可以在Orca中加载本地磁盘表。Orca提供`read_table`函数，通过该函数指定DolphinDB数据库和表名来使用DolphinDB数据表的数据函数支持的参数如下:
